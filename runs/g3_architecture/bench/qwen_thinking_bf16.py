"""Finite BF16 Qwen3.5-4B thinking reference. No labels, tools or output execution.
Same40fixture/sampling/prefix as Q4 thinking; excludes any censored completion.
BF16GGUF conversion, not low-bit quantization. Single sampling seed is not a ceiling.
"""
...
SHA='9e6e2841a75f503ccb330831832fd7861266e187e0dbf149a954219ccb8c197a'
...
def main():
 ...
 if len(rows)!=40 or any(set(z)!={'id','family','lang','prompt'} for z in rows):raise ValueError('exact40prompts,no gold')
 with tempfile.TemporaryDirectory(prefix='fg-g3-bf16-think-') as td:
  ...
  meta={'model':'Qwen/Qwen3.5-4B','weight_format':'BF16 GGUF, no low-bit quantization','model_sha256':SHA,'model_bytes':count,'llama_commit':revision,'thinking':True,'thinking_prefill':'<think>\n','sampling':sampling,'max_new_tokens':32768,'context':40960,'fixture_sha256':FIXTURE,'instruction_prefix':PREFIX,'tools':False,'single_sampling_seed_not_capability_ceiling':True,'run_id':os.environ.get('GITHUB_RUN_ID'),'source_commit':os.environ.get('GITHUB_SHA')}
  ...
  try:
   ...
   for i,z in enumerate(rows):
    ...
    req=urllib.request.Request('http://127.0.0.1:18080/completion',data=json.dumps({'prompt':prompt,'n_predict':32768,'seed':7401+i,'cache_prompt':False,**sampling}).encode(),headers={'Content-Type':'application/json'});t=time.perf_counter()
    ...
    print('FG_BF16_PROGRESS',i+1,row['tokens_predicted'],row['stop_type'],repr(final),flush=True)
   ...
   a.output.write_text(json.dumps(result,ensure_ascii=False,indent=2));a.output.with_suffix('.json.gz').write_bytes(gzip.compress(a.output.read_bytes(),mtime=0));print('FG_BF16_COMPLETE',json.dumps({'n':40,'censored':len(censored),'sha256':hashlib.sha256(a.output.read_bytes()).hexdigest()}),flush=True)
  finally:
   p.terminate()
